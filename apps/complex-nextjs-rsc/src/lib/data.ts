...
// In production, these would query the database via shared services
...
// Get user by ID
export async function getUserById(userId: string): Promise<(typeof seedData.users)[0] | null> {
  return seedData.users.find(u => u.id === userId) || null;
}
